...
/* These are uint32_t protocol constants, not C enum values. XQXI_END is
 * outside the range of a signed int and modern Clang correctly warns when it
 * is placed in an enum. */
#define XQXI_DMDUPLEX         UINT32_C(0x10000002)
#define XQXI_COPIES           UINT32_C(0x20000005)
#define XQXI_DMDEFAULTSOURCE  UINT32_C(0x20000006)
#define XQXI_DMMEDIATYPE      UINT32_C(0x20000000)
#define XQXI_RESOLUTION_X     UINT32_C(0x20000008)
#define XQXI_RESOLUTION_Y     UINT32_C(0x20000009)
#define XQXI_RASTER_X         UINT32_C(0x2000000d)
#define XQXI_RASTER_Y         UINT32_C(0x2000000e)
#define XQXI_VIDEO_BPP        UINT32_C(0x2000000a)
#define XQXI_VIDEO_X          UINT32_C(0x2000000f)
#define XQXI_VIDEO_Y          UINT32_C(0x20000010)
#define XQXI_ECONOMODE        UINT32_C(0x20000011)
#define XQXI_DMPAPER          UINT32_C(0x20000001)
#define XQXI_DUPLEX_PAUSE     UINT32_C(0x20000012)
#define XQXI_BIH              UINT32_C(0x40000002)
#define XQXI_END              UINT32_C(0x80000001)
...
be32(uint32_t dword)
{
    ...
    if (((char *)&probe)[0] == 1)
        return ((dword & UINT32_C(0x000000ff)) << 24)
             | ((dword & UINT32_C(0x0000ff00)) << 8)
             | ((dword & UINT32_C(0x00ff0000)) >> 8)
             | ((dword & UINT32_C(0xff000000)) >> 24);
    else
	return dword;
}
