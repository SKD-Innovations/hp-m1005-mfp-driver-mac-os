...
output_jbig(unsigned char *start, size_t len, void *cbarg)
{
    ...
    size_t	size = 65536;	// Printer does strange things otherwise.
...
start_doc(FILE *fp)
{
    static const char header[4] = {',', 'X', 'Q', 'X'};
    int		nitems;
    time_t	now;
    struct tm	time_parts;
    const char	*fixed_datetime;
    char	datetime[14+1];

    fixed_datetime = getenv("M1005_XQX_TIMESTAMP");
    if (fixed_datetime)
    {
	size_t i;
	if (strlen(fixed_datetime) != 14)
	    error(1, "M1005_XQX_TIMESTAMP must contain exactly 14 digits\n");
	for (i = 0; i < 14; ++i)
	    if (!isdigit((unsigned char)fixed_datetime[i]))
		error(1, "M1005_XQX_TIMESTAMP must contain exactly 14 digits\n");
	memcpy(datetime, fixed_datetime, 14 + 1);
    }
    else
    {
	now = time(NULL);
	if (localtime_r(&now, &time_parts) == NULL ||
	    strftime(datetime, sizeof(datetime), "%Y%m%d%H%M%S", &time_parts) == 0)
	    error(1, "Unable to create XQX job timestamp\n");
    }
...
cmyk_page(unsigned char *raw, int w, int h, FILE *ofp)
{
	...
	if (Debug >= 9)
	{
	    ...
	    snprintf(fname, sizeof(fname), "xxxplane%d", i);
	    dfp = fopen(fname, "w");
	    if (dfp)
	    {
...
main(int argc, char *argv[])
{
	...
	!((ResX == 600 && ResY == 600) || (ResX == 1200 && ResY == 600)))
	error(1, "Unsupported resolution %dx%d; use 600x600 or 1200x600\n",
	      ResX, ResY);
